...
const excluirLivro = async (req, res) => {
    ...
    try {
        ...
        const livroPendente = await conexao.query(`select * from livros left join emprestimos on emprestimos.id_livro = $1 and status = 'pendente'`, [id]);

        if (livroPendente.rowCount > 0) {
            return res.status(400).json('O livro não pode ser excluido porque está emprestado.');
        }
