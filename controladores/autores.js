...
const excluirAutor = async (req, res) => {
    ...
    try {
        ...
        const livroPendente = await conexao.query(`
        select autores.id as autor_id, livros.id as livro_id, 
        emprestimos.status from autores
        left join livros on livros.autor_id = autores.id
        left join emprestimos on emprestimos.id_livro = livros.id
        where autores.id = $1 and
        emprestimos.status = 'pendente'`, [id]);

        if (livroPendente.rowCount > 0) {
            return res.status(400).json('O autor possui livros pendentes, e não pode ser excluido');
        }

        const deleteLivro = await conexao.query(`delete from livros where autor_id = $1`, [id]);
        if (deleteLivro.rowCount === 0) {
            return res.status(400).json('Não foi possivel excluir os livros do autor solicitado.');
        }
